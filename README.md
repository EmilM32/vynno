...
```sh
# vynno-api — list https://vynno.localhost in SPA_ORIGIN, COOKIE_SECURE=true
./scripts/start
...
# open https://vynno.localhost
```

`./scripts/start --detach` writes `var/spa.pid` and `logs/spa.log`, and starts Caddy on loopback `:80` (redirect) and `:443` (TLS, HTTP/2 + HTTP/3). `./scripts/stop` stops Node and Caddy. `./scripts/status` and `./scripts/logs` are the operator view (JSON files, not the product `/logs` screen). `scripts/build` refuses while the SPA is running.

The Node process binds `127.0.0.1:27180`. Bookmark `https://vynno.localhost`, not `:27180` and not `127.0.0.1` — those origins do not share the session cookie. Needs `caddy` (`brew install caddy`). `*.localhost` resolves to loopback (RFC 6761); no `/etc/hosts` entry.
