...
| Login              | APG tabs (Log in / Create account). Visible labels. Login: `autocomplete="email"` / `current-password`. Register: `email` / `new-password` / `nickname`. Show/hide password toggles (`aria-pressed`). Client-only required errors via `role="alert"`. No paste blocking.                |
