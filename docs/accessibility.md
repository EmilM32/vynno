...
Axe runs every route under `dark`, `light`, `deep-dark`, `ember`, `newsprint`, and `zen` against `wcag2a`, `wcag2aa`, `wcag21aa`, and `wcag22aa`. The project dossier (`/projects/:id`) is covered by a dedicated case after navigating from the list.
...
| Live status        | `announce()` in `src/lib/a11y/announce.ts` — session start/stop only. **Never** put `aria-live` on the ticking clock.                                                                                                                                                                                                                                                |
...
| Insights range     | Period group (`aria-pressed` tabs) + prev/next `IconButton`s + visible range label (`aria-live="polite"`). Custom range is a `Dialog` of native `input type="date"` (`Field` + `Input`).                                                                                                                                                                             |
| Logs layout        | Entries / Grouped `PeriodToggle` (`role="group"`, `aria-pressed`) on `/logs` and project entries. Note and group expand `IconButton`s use `aria-expanded`.                                                                                                                                                                                                           |
| Weekly bars        | Named region + visually hidden `label: duration` list. LayerChart tooltip on hover.                                                                                                                                                                                                                                                                                  |
| Login              | APG tabs (Log in / Create account). Visible labels. Login: `autocomplete="email"` / `current-password`. Register: `email` / `new-password` / `nickname` / confirmation `one-time-code`. Forgot password: `email` / `one-time-code` / `new-password`. Show/hide password toggles (`aria-pressed`). Client-only required errors via `role="alert"`. No paste blocking. |
| Error pages        | Title `{page} · Vynno`. One `h1` (human title); status code is `aria-hidden`. Full-page 404/5xx has no `role="alert"` (the title change is enough) and no skip link (same as login). In-shell seed failure keeps `role="alert"`. Axe covers `/this-route-does-not-exist` in all three themes.                                                                        |
