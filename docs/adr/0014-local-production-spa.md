...
3. **The process is the built server** (`node --env-file=.env build`) on `127.0.0.1:27180` with `ORIGIN=http://vynno.local`. Build (`scripts/build`) and start (`scripts/start`) are separate so a restart does not rebuild.
