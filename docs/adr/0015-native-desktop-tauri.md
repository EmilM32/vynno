...
The UI is a SvelteKit app with SSR, `@sveltejs/adapter-node`, and a same-origin `/v1` BFF that forwards the HttpOnly `vynno_session` cookie to vynno-api ([0011](./0011-ssr-session-state.md), [0014](./0014-local-production-spa.md)). Daily use is a browser at `https://vynno.localhost`.
...
A packaged webview is not an HTTP origin. It cannot run the Node adapter, `+server.ts`, or `+layout.server.ts`. The session cookie cannot be first-party on `tauri://localhost` the way it is on `https://vynno.localhost`.
...
| Point Tauri at `https://vynno.localhost` forever             | Works as a spike (real HTTP origin, cookies unchanged). The `.app` still needs the Node SPA. Custom browser window, not a native app. |
