...
Repeatable frontend check of the **production UI** with the **chrome-devtools MCP**. You bring a running `https://vynno.localhost` and a login; an agent follows this doc. Findings stay in the chat unless you ask to save them.

Prompt to paste:

> Run the DevTools audit in `docs/devtools-audit.md`. Production is on https://vynno.localhost. Account is ready.

This is **not** a substitute for `npm test`, `npm run check`, or `npm run test:e2e` (axe). How to run axe: [accessibility.md](./accessibility.md). Screen inventory: [screens-and-flows.md](./screens-and-flows.md). How to boot `https://vynno.localhost`: [local-production.md](./local-production.md).
...
2. vynno-api up. `SPA_ORIGIN` includes `https://vynno.localhost`.
3. Browser URL is **`https://vynno.localhost`**, not `http://127.0.0.1` and not `http://vynno.localhost:27180` (different origins; cookie will not stick).
...
Confirm with a GET of `https://vynno.localhost/` before opening Chrome. `ERR_CONNECTION_REFUSED` is a setup fail.
...
Isolated context. Origin **`https://vynno.localhost`**. Record evidence (req id, insight name, snapshot uid) as you go.

### 0. Gate

1. `chrome-devtools__list_pages`.
2. `chrome-devtools__new_page` `url=https://vynno.localhost/login`, `isolatedContext` set (any stable name, e.g. `vynno-audit`).
...
`chrome-devtools__take_heapsnapshot` on `https://vynno.localhost` only:
