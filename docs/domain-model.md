...
This document describes the conceptual model the UI will implement. It is not a database schema. Backend models may diverge; the frontend will map API DTOs later.
...
| **Session / Time entry** | A timed interval. While running or paused it is the *active session*; when stopped it becomes a historical log entry. |
