...
| --- | --- | --- | --- |
| Active session card / Current Focus | `.pulse-border` (2s loop) | State indication | Brand signature. Paint cost accepted. |
...
Gate hover *transforms* with `@media (hover: hover) and (pointer: fine)`. Touch fires a false hover and leaves the element scaled. Color hovers do not need this gate.
