...
Implementations live in `src/lib/theme/tokens.css` (curves and durations) and `src/routes/layout.css` (`.press`, `.today-glow`).
...
| A solid border for a live session | Charts that draw themselves            |
...
- **State indication** — a live session is running (solid primary border on the timer card; `today-glow` on `StatusDot`)
